...
import {MockERC4626} from "./mocks/MockERC4626.t.sol";
...
contract ERC4626Test is DSTestPlus {
    ...
    function testSingleDepositWithdraw_allowance(uint128 amount, address spender) public {
        if (amount == 0) amount = 1;
        ...
        hevm.prank(alice);
        vault.approve(spender, aliceUnderlyingAmount);
    ...
    function testSingleDepositWithdraw_noAllowance(uint128 amount, address spender) public {
        if (amount == 0) amount = 1;
    ...
    function testSingleMintRedeem_allowance(uint128 amount, address spender) public {
        if (amount == 0) amount = 1;
        ...
        hevm.prank(alice);
        vault.approve(spender, aliceShareAmount);
    ...
    function testSingleMintRedeem_noAllowance(uint128 amount, address spender) public {
        if (amount == 0) amount = 1;
